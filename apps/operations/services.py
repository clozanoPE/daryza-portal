...
# ─── Etapa 3: Inspección Final (Adaptable para flujo CON_CALIDAD y SOLO_ALMACEN) ───
    ...
    @staticmethod
    def get_resumen_ticket(ticket_id: int) -> dict:
        """
        Devuelve un resumen completo del Ticket para el dashboard operativo.
        Ajustado para sincronización con detalle_ticket.html y optimización de carga.
        """
        try:
            ...
            raise ValidationError(f"No existe un Ticket con ID #{ticket_id}.")

        appointment = ticket.appointment

        # 1. Etapas ordenadas cronológicamente para el Dashboard
        etapas_data = []
        for stage in ticket.stages.all().order_by('fecha_inicio'):
            duracion = None
            if stage.fecha_fin:
                delta = stage.fecha_fin - stage.fecha_inicio
                mins = int(delta.total_seconds() // 60)
                duracion = f"{mins} min"
            
            etapas_data.append({
                'etapa': stage.get_etapa_display(),
                'etapa_codigo': stage.etapa,
                'inicio': stage.fecha_inicio.strftime('%H:%M:%S'),
                'fin': stage.fecha_fin.strftime('%H:%M:%S') if stage.fecha_fin else '—',
                'duracion': duracion or 'En curso',
                'usuario': stage.usuario.get_full_name() or stage.usuario.username,
            })

        # 2. Inspecciones (Sincronizado con detalle_ticket.html)
        inspecciones_data = {}
        for insp in ticket.inspections.all():
            etapa_key = insp.etapa
            if etapa_key not in inspecciones_data:
                inspecciones_data[etapa_key] = []
            
            inspecciones_data[etapa_key].append({
                'id': insp.id,
                'item_code': insp.po_line.item_code,
                'und_medida': insp.po_line.und_medida,  # <--- Sincronización necesaria
                'descripcion': insp.po_line.description,
                'oc_num': insp.po_line.purchase_order.doc_num,
                'tipo_oc': insp.po_line.purchase_order.u_mss_tdb or 'COMERCIAL',
                'cantidad_sap': str(insp.cantidad_sap),
                'cantidad_modificada': str(insp.cantidad_modificada),
                'estado': insp.estado,
                'requiere_coa': insp.requiere_coa,
                'coa_url': insp.coa_url or '', # coa_url es un URLField (string plano), no un FileField
                'comentario': insp.comentario or '',
                'usuario': insp.usuario.username if insp.usuario else 'Sistema',
            })

        # 3. Tiempo total con formato de horas si excede 60 min
        tiempo_str = "—"
        if ticket.tiempo_total_planta:
            total_mins = int(ticket.tiempo_total_planta.total_seconds() // 60)
            horas, mins = divmod(total_mins, 60)
        ...
        return {
            'ticket': {
                ...
                'tipo_flujo_display': ticket.get_tipo_flujo_display(),
                'codigo_qr': ticket.codigo_qr,
                'fecha_creacion': ticket.fecha_creacion.strftime('%d/%m/%Y %H:%M'),
            },
            'appointment': {
                'id': appointment.id,
                'fecha': appointment.slot.date.strftime('%d/%m/%Y'),
                'hora': appointment.slot.start_time.strftime('%H:%M'),
                'proveedor': appointment.user.get_full_name() or appointment.user.username,
                'sede': appointment.get_lugar_entrega_display(),
                'tiene_coa': bool(appointment.coa_pdf),
                'ocs': list(appointment.purchase_orders.values('doc_num', 'u_mss_tdb')),
            },
            'etapas': etapas_data,
