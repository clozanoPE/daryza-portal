...
def es_staff_interno(user):
    return user.is_authenticated and (
        user.groups.filter(name__in=['ALMACEN','CALIDAD','VIGILANCIA','COMPRAS']).exists()
...
def es_staff_o_proveedor(user):
    return user.is_authenticated and (
        user.groups.filter(name__in=['ALMACEN','CALIDAD','VIGILANCIA','COMPRAS','PROVEEDORES']).exists()
        or user.is_superuser
    )
