...
**Sesión 99 — el flujo de GRPO cambió: ahora hay un paso HUMANO intermedio en el Portal antes de que el daemon actúe.** Los artículos de Daryza se gestionan por LOTE, y el lote se ingresa al recibir la mercadería. Antes, la `EntradaMercaderia` se generaba automáticamente al finalizar el Ticket (Vigilancia registra la salida) y quedaba de inmediato disponible para el daemon (`estado_sap='L'`). Ahora nace en un estado nuevo `PENDIENTE` (`estado_sap=''`): Almacén / Materia Prima abre una pantalla de detalle en el Portal (mismo layout que "Copiar de OC" de Factura), completa el **número de lote** por línea más fechas de vencimiento/fabricación (opcionales), y recién entonces la **envía** (`estado` → `ENVIADO`, `estado_sap='' → 'L'`). Ahí el daemon la ve. **El GRPO sigue siendo de UN SOLO PASO hacia SAP** (`POST /PurchaseDeliveryNotes` directo, sin Draft) — lo único que cambió del lado del daemon es que cada `DocumentLine` ahora lleva `BatchNumbers` cuando la línea trae lote, y que se parsea también el `DocNum` visible de la respuesta (se guarda en `EntradaMercaderia.doc_num_sap` vía `confirmar-borrador`/`confirmar-definitivo`).

**Sesión 99b — el lote es obligatorio SOLO para los ítems que SAP realmente gestiona por lote.** `SyncSAPB1.GetLinesByDocEntry` ahora trae también `OITM.ManBtchNum` (columna `"Managed By Batch"`), que viaja en el payload de `/sync-oc/` como `gestionado_por_lote` (booleano por línea) y se persiste en `PurchaseOrderLine.gestionado_por_lote` del Portal. En la pantalla de Entrada de Mercadería, la columna LOTE solo es obligatoria/editable en las líneas con `gestionado_por_lote=True`; en las demás el campo queda deshabilitado y no se pide ni se envía. `services_entrada.enviar_a_sap` valida el lote solo en esas líneas. `GrpoService` ya enviaba `BatchNumbers` únicamente cuando `NumeroLote` no está vacío — con este cambio eso coincide naturalmente con "solo los ítems que lo necesitan", en vez de depender de que Almacén haya completado el campo.
...
| `daemon_data/SyncSAPB1.vb` | Consultas SQL directas contra HANA: `GetPendingHeaders` (OC con `U_DRYZ_PC='1'`), `GetLinesByDocEntry` (líneas + precio/IGV/moneda reales de SAP + **`OITM.ManBtchNum` como `"Managed By Batch"`**, sesión 99b — si el artículo se gestiona por lote), `MarkAsSynced` (única escritura: `UPDATE OPOR SET U_DRYZ_PC='2'`) | HANA (SQL directo, vía `HanaConnectionManager`) | Alimenta el payload de `POST /api/v1/sync-oc/` (no llama al Portal directamente — eso lo hace `SyncService`) |
...
**Actualización sesión 99c — CAMINO DE ÉXITO PROBADO por primera vez, con datos 100% reales.** Ciclo completo en el Portal local sobre la OC real `79001197` (`P20102021836` / ANTALIS TFM, ítem `EB00000455`, gestionado por lote) → Ticket `FINALIZADO` → Almacén completó el lote `LT-777` en el Portal → `--sync-grpo`:

```
10:02:05 | [GRPO-ÉXITO] Entrada #11: GRPO creado en SAP — DocEntry=44426, DocNum=16111114.
10:02:05 | [GRPO-CONFIRMADO] Entrada #11: borrador y definitivo confirmados en el Portal (DocEntry=44426, DocNum=16111114).
```

Verificado en SAP: GRPO `DocNum 16111114` real, `bost_Open`, `Cancelled=tNO`, `NumAtCard=79001197`, línea `EB00000455` qty 100 en almacén `EI01`; la OC `79001197` quedó `open 100/200` (recepción parcial). Verificado en el Portal: `EntradaMercaderia #11` → `estado='CREADO_SAP'`, `estado_sap='Y'`, `doc_entry_borrador`=`doc_entry_definitivo`=44426, `doc_num_sap='16111114'`.

**Hallazgo real que lo bloqueaba primero — `ManufactureDate` en `BatchNumbers` → `-4014`.** El primer `--sync-grpo` de la sesión 99c falló con `code -4014 "Cannot add row without complete selection of batch/serial numbers"`, aun con el lote completo. Aislado reproduciendo el `POST /PurchaseDeliveryNotes` exacto contra Service Layer, campo por campo del objeto `BatchNumbers`:

| Objeto `BatchNumbers[0]` enviado | Resultado |
|---|---|
| `{BatchNumber, Quantity}` | ✅ crea el GRPO (batch auto-creado con defaults) |
| `+ ExpiryDate` | ✅ crea el GRPO |
| `+ ManufactureDate` (solo, o junto a `ExpiryDate`, o + `AdmissionDate`) | ❌ `-4014` |

**`ManufactureDate` es el disparador** — la configuración de lotes de `BK_1808261700` no tiene "Fecha de fabricación" como atributo de lote habilitado, así que enviarlo hace que SAP considere la selección de lote incompleta/inválida. `ExpiryDate` sí se acepta. **Fix (Opción A, sesión 99c)**: `GrpoService.CrearGrpoAsync` dejó de enviar `ManufactureDate` — el `BatchNumbers` ahora es `{BatchNumber, Quantity, ExpiryDate?}`. `FechaFabricacionLote` se sigue recibiendo del Portal (para no romper el mapeo del DTO) pero se ignora al armar el payload; el dato queda guardado en `EntradaMercaderiaLinea.fecha_fabricacion_lote` del Portal, solo no se propaga a SAP. Rebuild MSBuild 0/0. Re-corrida → éxito (arriba).

**Nota de proceso**: aislar el `-4014` requirió 3 `POST /PurchaseDeliveryNotes` reales de prueba (DocEntry 44416/44418/44423) — los 3 se anularon (`POST .../Cancel`) inmediatamente, la OC volvió a `open 200/200` antes de la corrida real. Cuidar esto si se repite un diagnóstico así: cada POST de prueba crea un documento real que consume saldo de la OC.

**`--sync-grpo` sigue siendo flag manual, no `WorkerThread`** — el criterio no cambió por tener ahora una prueba de éxito: sigue pendiente decidir explícitamente el wireado al loop automático.
...
2. Almacén / Materia Prima (el actor de recepción del Ticket, según `Ticket.es_materia_prima`) abre una pantalla de detalle en el Portal (`/operations/entrada-mercaderia/<id>/`, mismo layout visual que "Copiar de OC" de Factura): ajusta la cantidad si hace falta, y completa el **número de lote** — **obligatorio SOLO en las líneas con `PurchaseOrderLine.gestionado_por_lote=True`** (`OITM.ManBtchNum` de SAP, sesión 99b); en las demás el campo LOTE queda deshabilitado — más fecha de vencimiento / fabricación (opcionales, también solo en las líneas gestionadas por lote).
3. Almacén hace clic en **"Enviar a SAP B1"** → `services_entrada.enviar_a_sap` valida que tengan lote las líneas **gestionadas por lote** (`gestionado_por_lote=True`, sesión 99b) y hace la transición `estado` `PENDIENTE → ENVIADO`, `estado_sap '' → 'L'`. Recién ahí el daemon la ve.
...
5. De vuelta en `GrpoSyncService.ProcesarEntradaAsync()`:
   ...
   - `Exitoso=False` → `ReportarErrorAsync(entrada.id, ...)` → `POST .../reportar-error/` con el mensaje real de SAP, sin tocar `estado`/`estado_sap` — la entrada sigue disponible para reintento, el ciclo continúa con la siguiente. `enviar_a_sap` (paso 5.1) ya exige lote en las líneas gestionadas por lote, así que el caso "falta el lote de un ítem que SÍ lo requiere" no debería llegar a SAP; un rechazo por otro motivo queda visible en `trazabilidad_ticket` para que un humano lo resuelva.
...
**Lo que NO cambia**: la capa HANA (`SyncSAPB1`) no participa — la creación del GRPO es 100% vía Service Layer, tal como establece la regla dura de la sección 1. **Atributos de lote (sesión 99c, resuelto)**: `BatchNumbers` envía `{BatchNumber, Quantity, ExpiryDate?}`. `ManufactureDate` **NO se envía** — `BK_1808261700` lo rechaza con `-4014` (no tiene "Fecha de fabricación" como atributo de lote habilitado); `ExpiryDate` sí se acepta. El batch se auto-crea en el GRPO con esos datos. Ver 3.3 para el detalle del hallazgo.
